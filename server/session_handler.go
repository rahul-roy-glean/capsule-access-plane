...
// SessionHandlers serves the session registration HTTP endpoints.
type SessionHandlers struct {
	sessions *SessionStore
	verifier identity.Verifier
}

// NewSessionHandlers creates session registration handlers.
func NewSessionHandlers(verifier identity.Verifier, store *SessionStore) *SessionHandlers {
...
func (h *SessionHandlers) RegisterSession(w http.ResponseWriter, r *http.Request) {
...
func (h *SessionHandlers) DeregisterSession(w http.ResponseWriter, r *http.Request) {
	if _, err := h.authenticate(r); err != nil {
		writeJSON(w, http.StatusUnauthorized, map[string]string{
			"error": err.Error(),
		})
...
func (h *SessionHandlers) authenticate(r *http.Request) (*identity.Claims, error) {
